...
def dtype_safe_training(model, criterion, img_tensor, targets, max_epochs=5):
    """数据类型安全的训练"""
    print(f"\n>>> 开始数据类型安全训练 (最多 {max_epochs} 轮)")
...
def main():
    print("=" * 80)
    print("===        RT-DETR数据类型修复验证        ===")
    print("===     专门解决float32/float64混合     ===")
    print("=" * 80)
    ...
    try:
        ...
        print("✅ 模型强制转换为float32")
        
        # 5. 创建损失函数
        criterion = build_criterion(num_classes)
        
        # 6. 数据类型安全训练
        model = dtype_safe_training(model, criterion, img_tensor, targets, max_epochs=5)
        
        print(f"\n" + "=" * 80)
        print("🎯 数据类型修复验证结果:")
        print("=" * 80)
        print("✅ 成功完成数据类型安全训练")
        print("✅ 解决了float32/float64混合问题")
        print("✅ RT-DETR Jittor版本基本功能正常")
        print("=" * 80)
