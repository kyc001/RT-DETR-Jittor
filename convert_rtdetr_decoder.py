...
class RTDETRTransformer(nn.Module):
    def __init__(self, num_classes=80, hidden_dim=256, num_queries=300,
                 position_embed_type='sine', feat_channels=[512, 1024, 2048], 
                 feat_strides=[8, 16, 32], num_levels=3, num_decoder_points=4, 
                 nhead=8, num_decoder_layers=6, dim_feedforward=1024, dropout=0.0,
                 activation="relu", num_denoising=100, label_noise_ratio=0.5,
...
print("=== 转换后的RT-DETR解码器Jittor代码 ===")
print(jittor_code)

# 保存转换后的代码
with open("jittor_rt_detr/src/nn/rtdetr_decoder_pytorch_aligned.py", "w") as f:
    f.write(jittor_code)
