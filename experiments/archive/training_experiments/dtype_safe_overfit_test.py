...
def main():
    ...
    try:
        ...
        # 最终测试
        print(f"\n=== 最终测试 ===")
        success, final_score = safe_inference_test(model, img_tensor, 100)
        
        # 保存模型
        if success:
            os.makedirs("checkpoints", exist_ok=True)
            jt.save(model.state_dict(), "checkpoints/rtdetr_dtype_safe.pkl")
            print("✅ 模型已保存到: checkpoints/rtdetr_dtype_safe.pkl")
