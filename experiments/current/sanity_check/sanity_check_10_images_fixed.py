...
def load_balanced_10_images():
    """加载复杂度平衡的10张训练图像"""
    print("🔄 加载复杂度平衡的10张训练图像...")
    ...
    # 按目标数量分组图像
    images_by_complexity = {}
    for img_info in coco_data['images']:
        image_id = img_info['id']
        annotations = image_annotations.get(image_id, [])
        if annotations:
            num_objects = len(annotations)
            if num_objects not in images_by_complexity:
                images_by_complexity[num_objects] = []
            images_by_complexity[num_objects].append((img_info, annotations))
    
    print(f"📊 按复杂度分组:")
    for num_objects in sorted(images_by_complexity.keys()):
        count = len(images_by_complexity[num_objects])
        print(f"   {num_objects}个目标: {count}张图像")
    
    # 选择复杂度相近的图像（2-6个目标）
    selected_data = []
    target_complexities = [2, 3, 4, 5, 6]  # 目标复杂度范围
    
    for complexity in target_complexities:
        if complexity in images_by_complexity:
            # 每个复杂度选择2张图像
            available_images = images_by_complexity[complexity]
            for i, (img_info, annotations) in enumerate(available_images[:2]):
    ...
    print(f"✅ 加载完成: {len(selected_data)}张复杂度平衡的训练图像")
    
    # 显示复杂度分布
    complexities = [data['num_objects'] for data in selected_data]
    complexity_counts = Counter(complexities)
    print(f"📊 选中图像的复杂度分布:")
    for complexity in sorted(complexity_counts.keys()):
        count = complexity_counts[complexity]
        print(f"   {complexity}个目标: {count}张图像")
...
def nms_filter(boxes, scores, classes, iou_threshold=0.5, score_threshold=0.25):
    """简单的NMS过滤（降低置信度阈值）"""
    if len(boxes) == 0:
        return [], [], []
...
def conservative_training_10_images(backbone, transformer, criterion, train_data, num_epochs=200):
    """保守的10张图像过拟合训练（解决模式崩塌）"""
    print(f"\n🚀 开始保守的10张图像过拟合训练 ({num_epochs}轮)")
    print("=" * 60)
    print("策略: 使用更低学习率和更多轮数，避免模式崩塌")
    ...
    try:
        ...
        # 创建优化器 - 使用更低的学习率
        optimizer = jt.optim.Adam(all_params, lr=8e-5, weight_decay=1e-5)
        ...
        print(f"   学习率: 8e-5 (降低以避免模式崩塌)")
        print(f"   权重衰减: 1e-5 (增加正则化)")
        ...
        print(f"   平均目标数: {total_objects/len(train_data):.1f}")
        ...
        for epoch in range(num_epochs):
            ...
            # 打印进度
            if epoch % 40 == 0 or epoch < 10 or epoch >= num_epochs - 10:
...
def postprocess_outputs(outputs, original_size, score_threshold=0.25):
    """后处理模型输出（降低置信度阈值以获得更多检测）"""
    try:
        # 获取预测结果
...
def test_and_visualize_fixed_model(backbone, transformer, train_data, score_threshold=0.25):
    """测试修复后的模型并生成可视化结果"""
    print(f"\n🧪 测试修复后的模型并生成可视化")
    print("=" * 60)
    print(f"验证: 是否解决了模式崩塌问题 (置信度阈值: {score_threshold})")
    ...
    try:
        ...
        # 创建可视化结果目录
        vis_dir = '/home/kyc/project/RT-DETR/results/sanity_check_10_images_fixed/visualizations'
        ...
        for i, data in enumerate(train_data):
            ...
            with jt.no_grad():
                ...
                # 生成可视化
                vis_save_path = os.path.join(vis_dir, f"fixed_image_{i+1:02d}_{data['file_name']}")
                title = f"Fixed Model - Image {i+1}: {data['file_name']}"
        ...
        image_predictions = []
        for i, data in enumerate(train_data):
            img_detections = [det for det in all_detections if det in all_detections[i*10:(i+1)*10]]  # 简化版本
            if len(all_detections) > 0:
                # 获取这张图像的检测类别
                start_idx = sum(len([d for d in all_detections[:j] if d]) for j in range(i))
                end_idx = start_idx + len([det for det in all_detections if det])
                # 简化：直接使用检测结果
                pass
        ...
        if class_counts:
            ...
            print(f"\n🎯 类别多样性分析:")
            print(f"   检测到的不同类别数: {unique_classes}")
            if unique_classes >= 5:
                print(f"   ✅ 类别多样性良好 - 模式崩塌已解决")
            elif unique_classes >= 3:
                print(f"   ⚠️ 类别多样性一般 - 部分改善")
            else:
                print(f"   ❌ 类别多样性不足 - 模式崩塌仍存在")
...
def save_model(backbone, transformer, losses, save_path):
    ...
    try:
        ...
        checkpoint = {
            ...
            'additional_info': {
                'training_method': 'sanity_check_10_images_fixed',
                'num_images': 10,
                'num_epochs': len(losses),
                'learning_rate': 8e-5,
                'optimizer': 'Adam',
                'score_threshold': 0.25,
                'data_strategy': 'complexity_balanced'
            }
        }
...
def main():
    print("🧪 RT-DETR 10张图像过拟合训练 (修复版)")
    print("解决模式崩塌问题：选择复杂度平衡的图像")
    print("=" * 60)

    # 1. 加载复杂度平衡的10张训练图像
    train_data = load_balanced_10_images()
    ...
    # 3. 保守的过拟合训练
    trained_backbone, trained_transformer, losses = conservative_training_10_images(
        backbone, transformer, criterion, train_data, num_epochs=200
    )
    ...
    # 4. 测试修复后的模型并生成可视化
    test_success = test_and_visualize_fixed_model(
        trained_backbone, trained_transformer, train_data, score_threshold=0.25
    )

    # 5. 保存模型
    save_path = '/home/kyc/project/RT-DETR/results/sanity_check_training/rtdetr_10img_fixed_200epoch.pkl'
    ...
    print(f"可视化结果保存在: /home/kyc/project/RT-DETR/results/sanity_check_10_images_fixed/visualizations/")
