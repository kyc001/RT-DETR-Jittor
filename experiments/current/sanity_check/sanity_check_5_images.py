#!/usr/bin/env python3
"""
基于ultimate_sanity_check.py的5张图像过拟合训练
逐步验证训练流程的可靠性
"""
...
def load_5_images():
    """加载5张训练图像（完全按照ultimate_sanity_check.py的方法）"""
    print("🔄 加载5张训练图像...")
    ...
    for img_info in coco_data['images'][:10]:  # 从前10张中选择5张有标注的
        ...
        try:
            ...
            for ann in annotations:
                # COCO格式: [x, y, width, height] -> 归一化的左上右下格式（完全按照ultimate_sanity_check.py）
                x, y, w, h = ann['bbox']
...
def visualize_results(image_data, detections, gt_annotations, save_path, title="Detection Results"):
    ...
    try:
        ...
        # 左侧：真实标注
        ax1.imshow(image)
        ax1.set_title(f'Ground Truth ({len(gt_annotations)}个目标)', fontsize=14, fontweight='bold')
        ax1.axis('off')
        ...
        # 右侧：检测结果
        ax2.imshow(image)
        ax2.set_title(f'Detection Results ({len(detections)}个检测)', fontsize=14, fontweight='bold')
        ax2.axis('off')
...
def intensive_training_5_images(backbone, transformer, criterion, train_data, num_epochs=100):
    """5张图像过拟合训练（基于ultimate_sanity_check.py的方法）"""
    print(f"\n🚀 开始5张图像过拟合训练 ({num_epochs}轮)")
    print("=" * 60)
    print("目标: 模型必须能够完美记住这5张图像的所有目标")
    ...
    try:
        ...
        for i, data in enumerate(train_data):
            print(f"   图像{i+1}: {data['file_name']} ({data['num_objects']}个目标)")
        ...
        for i, data in enumerate(train_data):
            ...
            print(f"   图像{i+1}: {total_loss.numpy().item():.4f}")
        ...
        for epoch in range(num_epochs):
            ...
            # 打印进度
            if epoch % 20 == 0 or epoch < 10 or epoch >= num_epochs - 10:
...
def test_overfitting_with_visualization(backbone, transformer, train_data, score_threshold=0.2):
    """测试过拟合效果并生成可视化结果"""
    print(f"\n🧪 测试过拟合效果并生成可视化")
    print("=" * 60)
    print("验证: 模型是否能在训练图像上检测到所有目标")
    ...
    try:
        ...
        # 创建可视化结果目录
        vis_dir = '/home/kyc/project/RT-DETR/results/sanity_check_5_images/visualizations'
        ...
        for i, data in enumerate(train_data):
            ...
            with jt.no_grad():
                ...
                # 判断是否成功（检测数量接近真实目标数量）
                if len(detections) >= len(gt_boxes) * 0.5:  # 至少检测到50%的目标
                    success_count += 1
                    print(f"   ✅ 过拟合成功")
                else:
                    print(f"   ❌ 过拟合不足")
...
def save_model(backbone, transformer, losses, save_path):
    ...
    try:
        ...
        checkpoint = {
            ...
            'additional_info': {
                'training_method': 'sanity_check_5_images',
                'num_images': 5,
...
def main():
    print("🧪 RT-DETR 5张图像过拟合训练")
    print("基于ultimate_sanity_check.py的逐步验证方法")
    print("=" * 60)
    
    # 1. 加载5张训练图像
    train_data = load_5_images()
    if len(train_data) < 5:
        print(f"❌ 只加载到{len(train_data)}张图像，需要5张")
        return
    ...
    # 3. 过拟合训练
    trained_backbone, trained_transformer, losses = intensive_training_5_images(
        backbone, transformer, criterion, train_data, num_epochs=100
    )
    ...
    # 4. 测试过拟合效果并生成可视化
    overfitting_success = test_overfitting_with_visualization(trained_backbone, trained_transformer, train_data)
    
    # 5. 保存模型
    save_path = '/home/kyc/project/RT-DETR/results/sanity_check_training/rtdetr_5img_100epoch.pkl'
