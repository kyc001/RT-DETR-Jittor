...
import jittor as jt
from jittor_rt_detr.src.zoo.rtdetr.build import build_rtdetr_model, build_rtdetr_criterion
...
def safe_inference_test(model, img_tensor, epoch):
    """数据类型安全的推理测试"""
    model.eval()
...
def main():
    ...
    try:
        ...
        model = build_rtdetr_model(num_classes=num_classes, hidden_dim=256, num_queries=300)
        print(f"✅ 使用标准构建函数创建模型，类别数: {num_classes}")
        
        # 使用标准构建函数创建损失函数
        criterion = build_rtdetr_criterion(num_classes)
        print("✅ 使用标准构建函数创建损失函数")
        
        # 优化器
        optimizer = jt.optim.AdamW(model.parameters(), lr=5e-5, weight_decay=1e-6)
        print("✅ 创建优化器，学习率: 5e-5")
        
        # 初始测试
        print("\n=== 训练前测试 ===")
        success, initial_score = safe_inference_test(model, img_tensor, 0)
        ...
        for epoch in range(1, 51):
            try:
                ...
                if epoch % 10 == 0 or epoch == 1:
                    ...
                    success, score = safe_inference_test(model, img_tensor, epoch)
                    if success:
        ...
        # 最终测试
        print(f"\n=== 最终测试 ===")
        success, final_score = safe_inference_test(model, img_tensor, 50)
        ...
        print("=" * 60)
        print("✅ 文件结构已完全对齐PyTorch版本")
        print("✅ 使用标准的构建函数和模块组织")
