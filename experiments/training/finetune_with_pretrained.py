...
def load_pretrained_weights(model, pretrained_path, strict=False):
    ...
    try:
        pretrained_dict = jt.load(pretrained_path)
        ...
        for key in model_dict.keys():
            if key in pretrained_dict:
                if model_dict[key].shape == pretrained_dict[key].shape:
                    ...
                    print(f"⚠️ 形状不匹配: {key} - 模型: {model_dict[key].shape}, 预训练: {pretrained_dict[key].shape}")
...
def train_with_pretrained(pretrained_path, num_epochs=20, learning_rate=1e-5):
    """使用预训练权重进行微调训练"""
    print("=" * 60)
    ...
    print(f"开始微调训练 - 轮数: {num_epochs}, 学习率: {learning_rate}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='使用预训练权重微调RT-DETR')
    ...
    if not os.path.exists(args.pretrained):
        print(f"❌ 预训练权重文件不存在: {args.pretrained}")
        sys.exit(1)
    
    success = train_with_pretrained(args.pretrained, args.epochs, args.lr)
