...
严格按照PyTorch版本: rtdetr_pytorch/src/zoo/rtdetr/rtdetr_decoder.py
"""
...
from .utils import deformable_attention_core_func, get_activation, inverse_sigmoid, bias_init_with_prob, tile
...
class MSDeformableAttention(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        self.sampling_offsets.bias = jt.array(grid_init.flatten().numpy())
    ...
    def execute(self, query, reference_points, value, value_spatial_shapes, value_mask=None):
        ...
        Args:
            ...
            reference_points: [bs, query_length, n_levels, 2], range in [0, 1]
            value: [bs, value_length, C]
            value_spatial_shapes: List of [n_levels, 2], [(H_0, W_0), ...]
            value_mask: [bs, value_length], True for non-padding elements
        """
        ...
        if value_mask is not None:
            value_mask = value_mask.float32().unsqueeze(-1)
        ...
        attention_weights = jt.nn.softmax(attention_weights, dim=-1).reshape(
            bs, Len_q, self.num_heads, self.num_levels, self.num_points)

        if reference_points.shape[-1] == 2:
            offset_normalizer = jt.array(value_spatial_shapes, dtype=jt.float32)
        ...
        output = deformable_attention_core_func(value, value_spatial_shapes, None, sampling_locations, attention_weights)
...
class MultiHeadAttention(nn.Module):
    ...
    def execute(self, query, key, value, attn_mask=None):
        ...
        q = self.q_proj(query).view(bs, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.k_proj(key).view(bs, key_len, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.v_proj(value).view(bs, key_len, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        if attn_mask is not None:
            ...
            if attn_mask.dtype == jt.bool:
                scores = jt.where(attn_mask, jt.array(-float('inf')), scores)
            else:
        ...
        out = out.transpose(1, 2).contiguous().view(bs, seq_len, self.embed_dim)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def execute(self, tgt, reference_points, memory, memory_spatial_shapes,
        ...
        tgt = self.norm3(jt.clamp(tgt, min_v=-65504, max_v=65504))
...
class TransformerDecoder(nn.Module):
    ...
    def execute(self, tgt, ref_points_unact, memory, memory_spatial_shapes,
                memory_level_start_index, bbox_head, score_head, query_pos_head,
                attn_mask=None, memory_mask=None):
        output = tgt
        dec_out_bboxes = []
        dec_out_logits = []
        # 使用stop_grad()与变量名保持一致，防止梯度回传到初始ref_points
        ...
        for i, layer in enumerate(self.layers):
            ...
            inter_ref_bbox = jt.sigmoid(bbox_head[i](output) + inverse_sigmoid(ref_points_detach))
            ...
            if self.is_training():
                ...
                if i == 0:
                    ...
                    dec_out_bboxes.append(jt.sigmoid(bbox_head[i](output) + inverse_sigmoid(ref_points)))
            ...
            ref_points_detach = inter_ref_bbox.stop_grad() if self.is_training() else inter_ref_bbox

        return jt.stack(dec_out_bboxes), jt.stack(dec_out_logits)
...
class RTDETRTransformer(nn.Module):
    ...
    def _generate_anchors(self, spatial_shapes=None, grid_size=0.05, dtype=jt.float32):
        ...
        for lvl, (h, w) in enumerate(spatial_shapes):
            grid_y, grid_x = jt.meshgrid(
                jt.arange(h, dtype=dtype),
                jt.arange(w, dtype=dtype))
            grid_xy = jt.stack([grid_x, grid_y], -1)
            valid_WH = jt.array([w, h], dtype=dtype)
            grid_xy = (grid_xy.unsqueeze(0) + 0.5) / valid_WH
            wh = jt.ones_like(grid_xy) * grid_size * (2.0 ** lvl)
            anchors.append(jt.concat([grid_xy, wh], -1).reshape(-1, h * w, 4))
        ...
        anchors = jt.where(valid_mask, anchors, jt.array(float('inf')))
    ...
    def _get_decoder_input(self, memory, spatial_shapes,
        ...
        if self.is_training() or self.eval_spatial_size is None:
            anchors, valid_mask = self._generate_anchors(spatial_shapes)
        else:
            anchors, valid_mask = self.anchors, self.valid_mask

        memory = valid_mask.float32() * memory
        ...
        topk_ind_expand_logits = topk_ind.unsqueeze(-1).expand([bs, self.num_queries, enc_outputs_class.shape[-1]])
        enc_topk_logits = jt.gather(enc_outputs_class, 1, topk_ind_expand_logits)

        # extract region features
        if self.learnt_init_query:
            target = tile(self.tgt_embed.weight.unsqueeze(0), [bs, 1, 1])
        else:
            topk_ind_expand_mem = topk_ind.unsqueeze(-1).expand([bs, self.num_queries, output_memory.shape[-1]])
            target = jt.gather(output_memory, 1, topk_ind_expand_mem)
            target = target.stop_grad()

        if denoising_class is not None:
            target = jt.concat([denoising_class, target], 1)

        return target, reference_points_unact.stop_grad(), enc_topk_bboxes, enc_topk_logits
    ...
    def execute(self, feats, targets=None):
        ...
        if self.is_training() and self.num_denoising > 0:
        ...
        if self.is_training() and dn_meta is not None:
            dn_out_bboxes, out_bboxes = jt.split(out_bboxes, dn_meta['dn_num_split'], dim=2)
            dn_out_logits, out_logits = jt.split(out_logits, dn_meta['dn_num_split'], dim=2)

        out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1]}

        if self.is_training() and self.aux_loss:
