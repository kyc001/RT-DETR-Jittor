...
def detailed_inference_analysis(img_path, model_path):
    """详细的推理分析"""
    print(f"\n🔍 详细推理分析...")
    ...
    print(f"  - Logits均值: {np.mean(logits_np):.3f}")
    print(f"  - Logits标准差: {np.std(logits_np):.3f}")
    ...
    print(f"  - 主要预测类别:")
    for i, (class_idx, count) in enumerate(class_counts[:10]):
    ...
    for threshold in thresholds:
        ...
        print(f"  阈值 {threshold:.2f}: {valid_count} 个候选")
        
        if valid_count > 0 and valid_count < 50:  # 选择合理数量的检测
    ...
    print(f"\n🎯 使用阈值 {best_threshold:.2f} 进行检测:")
    print(f"  候选数量: {len(valid_indices)}")
    ...
    for det in all_detections:
        ...
        if not is_duplicate:
            ...
            # 限制总检测数量
            if len(filtered_detections) >= 15:
                break
    
    return image, filtered_detections

def create_detailed_visualization(image, gt_data, detections, img_name):
    """创建详细可视化"""
    ...
    # 右图：检测结果
    ax2.imshow(image)
    ax2.set_title(f"Balanced Model Results\n{len(detections)} detections (low threshold)", fontsize=16, fontweight='bold')
    ax2.axis('off')
    ...
    for i, det in enumerate(detections):
        ...
        ax2.text(x1, y1-5, f"P{i+1}: {det['class_name']} ({det['confidence']:.3f})", 
                bbox=dict(boxstyle="round,pad=0.3", facecolor=color, alpha=0.8),
    ...
    # 保存结果
    output_name = f"detailed_balanced_{img_name.replace('.jpg', '')}.jpg"
    plt.savefig(output_name, dpi=150, bbox_inches='tight')
    plt.close()
    
    print(f"✅ 详细可视化已保存: {output_name}")
    return output_name
...
def main():
    ...
    image, detections = detailed_inference_analysis(img_path, model_path)
    
    # 4. 分析结果
    gt_counts, pred_counts = analyze_detection_results(gt_data, detections)
    
    # 5. 创建可视化
    output_file = create_detailed_visualization(image, gt_data, detections, img_name)
    ...
    if detections:
        ...
        correct_classes = expected_classes & detected_classes
        missing_classes = expected_classes - detected_classes
        extra_classes = detected_classes - expected_classes
        
        print(f"  ✅ 正确检测的类别: {', '.join(correct_classes) if correct_classes else '无'}")
        print(f"  ❌ 缺失的类别: {', '.join(missing_classes) if missing_classes else '无'}")
        print(f"  ⚠️ 额外检测的类别: {', '.join(extra_classes) if extra_classes else '无'}")
        
        # 计算类别准确率
        if expected_classes:
            class_recall = len(correct_classes) / len(expected_classes)
            print(f"  📈 类别召回率: {class_recall:.2%}")
    else:
        print(f"  ❌ 没有检测到任何目标")
